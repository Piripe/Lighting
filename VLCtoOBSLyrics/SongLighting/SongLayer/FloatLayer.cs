...
{
    public class FloatLayer : ISongLayer
    {
        [JsonProperty("type")]
        public SongLayerType Type { get => SongLayerType.Float; }
        [JsonProperty("action")]
        ...
        public Dictionary<int,KeyframeData<float>> Keyframes { get; set; } = new Dictionary<int, KeyframeData<float>>();


        public object GetValueAtFrame(int frame)
        {
            return GetFloatAtFrame(frame);
        }
        public float GetFloatAtFrame(int frame)
        {
            ...
            KeyframeData<float> originKeyframeData = new KeyframeData<float>(Ease.None, 0);
            int nextKeyframe = 0;
            KeyframeData<float> nextKeyframeData = new KeyframeData<float>(Ease.None, 0);

            if (originValidKeyframes.Count == 0)
            {
                if (Keyframes.Count != 0)
                {
                    originKeyframe = Keyframes.Min((KeyValuePair<int, KeyframeData<float>> x) => x.Key);
            ...
            if (nextValidKeyframes.Count == 0)
            {
                if (Keyframes.Count != 0)
                {
                    nextKeyframe = Keyframes.Min((KeyValuePair<int, KeyframeData<float>> x) => x.Key);
            ...
            return EaseHelper.GetEaseValue(originKeyframe, originKeyframeData, nextKeyframe, nextKeyframeData, frame);
        }
    }
}
